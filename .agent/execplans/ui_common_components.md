...
### 類似ケース洗い出しを「必ず行う」ための原則
- 単一箇所の修正依頼を受けたら、同種の実装パターンをリポジトリ全体で検索し、同時に直すか「未対応リスト」として記録する。例: 文言変更・バリデーション追加・レイアウト統一など。
- 検索の起点を事前に決める: `rg 'name="search"'`, `rg 'FloatingInput'`, `rg 'searchLabel'` 等のキーワードをテンプレ化し、作業開始時に必ず実行。
- ExecPlan/PRに「類似箇所の確認結果」を短く記載（全対応 / 未対応あり→理由と対応予定）。
- これを守れない場合（時間制約など）は、その旨を Decision Log に残し、後続タスクとして Backlog に積む。
